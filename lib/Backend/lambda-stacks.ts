...
export class lambdastack extends cdk.Stack {
  ...
  public readonly addToReadingListLambda: lambda.Function; //not integrated yet

  public readonly getReview: lambda.Function;
  public readonly createReview: lambda.Function;
  ...
  constructor(
    ...
    props?: cdk.StackProps & { synthesisMode?: boolean }
  ) {
    ...
    const textExtractionLambda = new lambda.Function(
      this,
    ...
    const textractNotificationTopic = new sns.Topic(
      this,
    ...
    const startTextractJobLambda = new lambda.Function(
      this,
    ...
    splitChaptersLambda.addToRolePolicy(
    ...
    splitChaptersLambda.addEnvironment(
      "CHAPTERS_TABLE",
      dbStack.chapter.tableName
    );
    dbStack.chapter.grantWriteData(splitChaptersLambda);
    splitChaptersLambda.addEnvironment(
      "READING_MATERIALS_BUCKET",
      StorageStack.readingMaterials.bucketName
    );
    ...
    const generateSummaryLambda = new lambda.Function(
      this,
      "GenerateSummaryLambda",
      {
        runtime: lambda.Runtime.NODEJS_18_X,
        handler: "index.handler",
    ...
    generateSummaryLambda.addEventSource(
      new lambdaEventSources.SqsEventSource(summaryQueue)
    );
    ...
    const generateScriptLambda = new lambda.Function(
      this,
    ...
    generateScriptLambda.addToRolePolicy(
      new iam.PolicyStatement({
    ...
    generateScriptLambda.addEventSource(
      new lambdaEventSources.SqsEventSource(scriptQueue)
    );
    ...
    updateBookLambda.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["dynamodb:Query"],
        resources: [
          `arn:aws:dynamodb:${this.region}:${this.account}:table/${dbStack.book.tableName}/index/GSI_by_book_id`,
    ...
    generateSSMLLambda.addEventSource(
      new lambdaEventSources.SqsEventSource(ssmlQueue)
    );
    ...
    this.getUploadUrlsLambda = new lambda.Function(
      this,
      "GetUploadUrlsLambda",
      {
        runtime: lambda.Runtime.NODEJS_18_X,
    ...
    generateAudioLambda.addEventSource(
      new lambdaEventSources.SqsEventSource(pollyQueue)
    );
    pollyQueue.grantConsumeMessages(generateAudioLambda);

    generateAudioLambda.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
    ...
    finalVideoLambda.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          "s3:GetObject",
          "s3:PutObject",
    ...
    finalVideoLambda.addEventSource(
      new lambdaEventSources.SqsEventSource(audioMergeQueue)
    );
    ...
    // Permissions
    StorageStack.readingMaterialsQueue.grantConsumeMessages(
      textExtractionLambda
    );
    ...
    const saveExtractedTextLambda = new lambda.Function(
      this,
    ...
    // Permissions
    StorageStack.extractedTextQueue.grantConsumeMessages(
      saveExtractedTextLambda
    ); //this permission will allow lambda function to consume the messages from the extractedTextQueue
    ...
    saveExtractedTextLambda.addEventSource(
      new lambdaEventSources.SqsEventSource(StorageStack.extractedTextQueue)
    ); //
    ...
    this.BedRockFunction.addEventSource(
      new lambdaEventSources.SqsEventSource(videoScriptQueue, {
        batchSize: 1,
    ...
    (this.messageProcessing = new lambda.Function(
      this,
      ...
      (this.invokeBedrockLib = new lambda.Function(
        this,
        "InvokeBedrockLibrarianLambda",
        {
          runtime: lambda.Runtime.NODEJS_18_X,
    ...
    if (!synthesisMode) {
      ...
      try {
        ...
        console.warn(
          "Skipping event notification setup during synthesis to avoid circular dependencies"
        );
      }
    } else {
      console.log(
        "Running in synthesis mode - skipping event notification setup"
      );
    }
    ...
    this.invokeBedrock.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          "bedrock:*",
          "logs:CreateLogGroup",
    ...
    this.invokeBedrockLib.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          "bedrock:*",
          "logs:CreateLogGroup",
    ...
    this.messageProcessing.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          "bedrock:*",
          "transcribe:*",
    ...
    this.playResponse.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          "bedrock:*",
          "logs:CreateLogGroup",
    ...
    this.transcribe.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          "bedrock:*",
          "transcribe:*",
    ...
    this.triggerPolly.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          "bedrock:*",
          "polly:*",
    ...
    this.deleteBookLambdav2.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["dynamodb:*"],
        resources: [
          "arn:aws:dynamodb:us-east-1:672461264983:table/DBStack-bookF0785129-1B9WR0J1EB4DN",
        ],
      })
    );
    ...
    this.addToReadingListLambda = new lambda.Function(
      this,
      "AddToReadingListLambda",
      {
        runtime: lambda.Runtime.NODEJS_18_X,
        handler: "index.handler",
        code: lambda.Code.fromAsset("lambda/AddToList"),
        environment: {
          BOOK_TABLE_NAME: dbStack.reader_books.tableName,
        },
      }
    );

    // Grant permissions to access the DynamoDB table
    dbStack.reader_books.grantReadWriteData(this.addToReadingListLambda);
    dbStack.reader_books.grantFullAccess(this.addToReadingListLambda);
    dbStack.book.grantReadWriteData(this.addToReadingListLambda);

    this.createReview = new lambda.Function(this, "CreateReviewLambda", {
      runtime: lambda.Runtime.NODEJS_18_X,
      handler: "index.handler",
      code: lambda.Code.fromAsset("lambda/CreateReview"),
      environment: {
        REVIEW_TABLE_NAME: dbStack.reviews.tableName,
      },
    });
    this.getReview = new lambda.Function(this, "GetReviewLambda", {
      runtime: lambda.Runtime.NODEJS_18_X,
      handler: "index.handler",
      code: lambda.Code.fromAsset("lambda/GetReview"),
      environment: {
        REVIEW_TABLE_NAME: dbStack.reviews.tableName,
      },
    });
    // Grant permissions to access the DynamoDB table
    dbStack.reviews.grantReadWriteData(this.createReview);
    dbStack.reviews.grantReadData(this.getReview);
  }
}
