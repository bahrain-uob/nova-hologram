...
exports.handler = async (event) => {
  try {
    ...
    const bookUrl = await getSignedUrl(
      s3Client,
      new PutObjectCommand({ Bucket: BUCKET_NAME, Key: bookKey, ContentType: "application/pdf" }),
      { expiresIn }
    );

    const coverUrl = await getSignedUrl(
      s3Client,
      new PutObjectCommand({ Bucket: BUCKET_NAME, Key: coverKey, ContentType: "image/jpeg" }),
      { expiresIn }
    );
