...
export default function AddBookPage() {
  ...
  const fetchBookData = async () => {
    ...
    try {
      ...
      if (isbnInput.trim()) {
        // Search by ISBN
        url = `https://openlibrary.org/api/books?bibkeys=ISBN:${isbnInput.trim()}&format=json&jscmd=data`;
      } else if (title.trim()) {
        // Search by title
      ...
      if (isbnInput.trim()) {
        const key = `ISBN:${isbnInput.trim()}`;
        bookInfo = data[key];
      } else if (title.trim() && data.docs && data.docs.length > 0) {
        bookInfo = data.docs[0]; // Take the first match
      }
      ...
      if (bookInfo) {
        setBookData({
          ...
          authors: bookInfo.authors
            ? bookInfo.authors.map((a: any) => a.name)
            : bookInfo.author_name || [],
          publisher: bookInfo.publishers
            ? bookInfo.publishers[0].name
            : bookInfo.publisher?.[0] || "",
          publishedDate:
            bookInfo.publish_date ||
            bookInfo.first_publish_year?.toString() ||
            "",
          description:
            bookInfo.description?.value || bookInfo.description || "",
          maturityRating: "", // Open Library doesn't have maturity rating directly
          imageLinks: {
            thumbnail: bookInfo.cover?.medium
      ...
      <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <Select value={collection} onValueChange={setCollection}>
                    ...
                    <SelectContent>
                      <SelectItem value="summer-reading">
                        Summer Reading
                      </SelectItem>
        ...
        <div className="flex justify-end gap-4 mt-8">
          ...
          <Button
            ...
            onClick={
              isFetched ? () => router.push("/videosGenerated") : fetchBookData
            }
            disabled={loading}
          >
