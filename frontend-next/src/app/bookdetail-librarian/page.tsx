    ...
    <div className="mb-4">
      ...
      {editMode ? (
        ...
        <p className="text-sm text-gray-900 whitespace-pre-wrap">
          {value || "-"}
        </p> // ← removed bg, border, and padding
      )}
    </div>
  );
}
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setCollection(
          typeof book.collection === "string" ? book.collection : undefined
        );
        ...
        if (Array.isArray(book.objectives)) {
          if (typeof book.objectives[0] === "string") {
            ...
            setObjectives(
              book.objectives.map((text, index) => ({ id: index + 1, text }))
            );
          } else if (
            typeof book.objectives[0] === "object" &&
            book.objectives[0].text
          ) {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const payload = {
        book_id: bookId,
        updatedFields,
        chapters: chapters.map(({ chapter_id, summary }) => ({
          chapter_id,
          summary,
        })),
      };
      ...
      const res = await fetch(
        "https://hfnngonop5.execute-api.us-east-1.amazonaws.com/update-book",
        {
          method: "PUT",
          headers: {
          ...
          <div className="col-span-1">
            <Card className="bg-white shadow p-4 h-full">
              <CardContent className="pt-6 flex flex-col items-center">
                <h3 className="text-sm font-medium text-gray-600 mb-2 self-start">
                  Book Cover
                </h3>
                <br></br>
                {uploadedImage ? (
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <SelectWithLabel
                      ...
                      onChange={setType}
                      editMode={editMode}
                      options={["fiction", "non-fiction", "textbook"]}
                    />
                  </div>
                  <div>
                    <SelectWithLabel
                      ...
                      onChange={setGenre}
                      editMode={editMode}
                      options={["mystery", "science", "history"]}
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <SelectWithLabel
                      ...
                      editMode={editMode}
                      options={["summer-reading", "classics", "bestsellers"]}
                    />
                  </div>
                  <div>
                    <FieldWithLabel
                ...
                <div className="mb-4">
                  ...
                  {editMode ? (
                    ...
                    <div className="pl-3 border-l-2 border-gray-200 space-y-1">
                      {objectives.length > 0 ? (
                        objectives.map((obj) => (
                          <p key={obj.id} className="text-sm text-gray-900">
                            - {obj.text}
                          </p>
                        ))
                      ) : (
                ...
                {loading ? (
                  ...
                  bookData.chapters.map((chapter, index) => (
                    ...
                    >
                      ...
                      <div className=" flex items-center justify-center">
                        {chapter.trailer_status === "completed" &&
                        chapter.trailer ? (
                          <video
                            controls
                            className="rounded-md w-full max-w-4xl aspect-video"
